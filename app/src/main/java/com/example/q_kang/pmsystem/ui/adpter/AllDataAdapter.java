...
public class AllDataAdapter extends BaseQuickAdapter<AllShowData, BaseViewHolder> {
    private Context context;
    private List<AllShowData> data;


    public AllDataAdapter(Context context, @Nullable List<AllShowData> data) {
        super(R.layout.all_data_layout, data);
    ...
    protected void convert(BaseViewHolder helper, AllShowData item) {
        ...
        helper.setText(R.id.tv_all_time, item.getTime());
        String name = item.getName();
        if (null == name || "".equals(name) || "null".equals(name)) {
            name = "暂无名称";
        }
        helper.setText(R.id.tv_all_name, name);

        ImageView iv_classify = helper.getView(R.id.iv_all_classify);
        if (flag == 1) {
        ...
        RoundLinearLayout rll_all = helper.getView(R.id.rll_all);
