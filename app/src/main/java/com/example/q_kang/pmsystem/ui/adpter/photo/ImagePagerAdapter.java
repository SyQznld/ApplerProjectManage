...
public class ImagePagerAdapter extends PagerAdapter {
    ...
    List<ImageItemBean> mImgList;
    private OnItemClickListener mListener;

    public ImagePagerAdapter(Context context, List<ImageItemBean> imgList) {
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        final ImageItemBean image = mImgList.get(position);
        container.addView(currentView);
        Glide.with(mContext).asBitmap()
                .apply(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE))
                .load(new File(image.getImagePath())).into(new SimpleTarget<Bitmap>() {
            @Override
    ...
    public interface OnItemClickListener {
        void onItemClick(int position, ImageItemBean image);
    }
